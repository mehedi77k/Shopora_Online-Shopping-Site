...
$sales=$pdo->query("SELECT COALESCE(SUM(total_amount),0) bdt, COALESCE(SUM(COALESCE(total_eur,total_amount/NULLIF(eur_exchange_rate,0))),0) eur FROM orders WHERE order_status <> 'Cancelled'")->fetch();
$today=$pdo->query("SELECT COALESCE(SUM(total_amount),0) bdt, COALESCE(SUM(COALESCE(total_eur,total_amount/NULLIF(eur_exchange_rate,0))),0) eur FROM orders WHERE order_status <> 'Cancelled' AND DATE(order_date)=CURDATE()")->fetch();
$avg=$pdo->query("SELECT COALESCE(AVG(total_amount),0) bdt, COALESCE(AVG(COALESCE(total_eur,total_amount/NULLIF(eur_exchange_rate,0))),0) eur FROM orders WHERE order_status <> 'Cancelled'")->fetch();
...
$currentRate=current_eur_rate($pdo);
...
<div class="admin-rate-bar"><div><span>Reference exchange rate</span><strong><?= $currentRate ? '€1 = '.money($currentRate) : 'EUR rate not configured' ?></strong></div><?php if(is_super_admin()): ?><a href="<?= url('admin/currency_settings.php') ?>">Manage currency →</a><?php else: ?><small>Managed by Super Admin</small><?php endif; ?></div>
<div class="metric-grid metric-grid-finance">
<div class="metric-card"><div class="label">TOTAL SALES</div><div class="value"><?= money($sales['bdt']) ?></div><div class="secondary-value"><?= eur_money($sales['eur']) ?></div><div class="trend">Non-cancelled orders</div></div>
<div class="metric-card"><div class="label">TODAY'S SALES</div><div class="value"><?= money($today['bdt']) ?></div><div class="secondary-value"><?= eur_money($today['eur']) ?></div><div class="trend">Today, non-cancelled</div></div>
<div class="metric-card"><div class="label">AVERAGE ORDER</div><div class="value"><?= money($avg['bdt']) ?></div><div class="secondary-value"><?= eur_money($avg['eur']) ?></div><div class="trend">Non-cancelled orders</div></div>
