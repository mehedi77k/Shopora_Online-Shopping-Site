...
$orderRate=order_rate($order); $orderEur=order_eur_amount($order);
...
<?php foreach ($items as $item): $itemEur=$orderRate ? round((float)$item['subtotal']/$orderRate,2) : null; ?><div class="cart-item"><img class="cart-thumb" src="<?= e(product_image($item['image'])) ?>" alt=""><div><h3><?= e($item['product_name'] ?: 'Product') ?></h3><span class="muted"><?= (int)$item['quantity'] ?> × <?= dual_money($item['unit_price'], $orderRate ? round((float)$item['unit_price']/$orderRate,2) : null, $orderRate, false) ?></span></div><strong><?= dual_money($item['subtotal'],$itemEur,$orderRate,false) ?></strong></div><?php endforeach; ?>
<div class="content-card" style="padding:22px"><h3>Shipping address</h3><p class="muted"><?= nl2br(e($order['shipping_address'])) ?></p></div></div>
<aside class="summary-card"><h3>Summary</h3><div class="summary-row"><span>Order status</span><span class="status <?= strtolower($order['order_status']) ?>"><?= e($order['order_status']) ?></span></div><div class="summary-row"><span>Payment method</span><strong><?= e($order['payment_method']) ?></strong></div><div class="summary-row"><span>Payment status</span><span class="status <?= strtolower($payment['payment_status'] ?? 'pending') ?>"><?= e($payment['payment_status'] ?? 'Pending') ?></span></div><div class="summary-row total"><span>Total</span><span><?= dual_money($order['total_amount'],$orderEur,$orderRate,false) ?></span></div><?php if ($orderRate): ?><div class="currency-checkout-note"><strong>Exchange rate at purchase</strong><span>€1 = <?= money($orderRate) ?></span><small>This historical rate is locked to this order.</small></div><?php endif; ?><a class="btn btn-ghost btn-block" href="<?= url('account.php') ?>">Back to account</a></aside></div></section>
