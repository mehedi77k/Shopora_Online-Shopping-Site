...
if (!$currentRate) {
    $errors[] = 'EUR reference rate is not configured yet. Please contact the store administrator before checkout.';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $currentRate = current_eur_rate($pdo);
    if (!$currentRate) $errors[] = 'EUR reference rate is not configured yet.';
    ...
    if (!$errors) {
        try {
            ...
            $rateStmt = $pdo->prepare("SELECT rate_to_bdt FROM currency_rates WHERE currency_code='EUR' AND is_active=1 FOR UPDATE");
            $rateStmt->execute();
            $lockedRate = (float)$rateStmt->fetchColumn();
            if ($lockedRate <= 0) throw new RuntimeException('EUR exchange rate is unavailable.');
            $totalEur = round($calculatedTotal / $lockedRate, 2);

            $orderStmt = $pdo->prepare("INSERT INTO orders (user_id,total_amount,base_currency,eur_exchange_rate,total_eur,shipping_address,payment_method,order_status) VALUES (?,?,?,?,?,?,?, 'Pending')");
            $orderStmt->execute([(int)$_SESSION['user']['user_id'],$calculatedTotal,'BDT',$lockedRate,$totalEur,$address,$paymentMethod]);
